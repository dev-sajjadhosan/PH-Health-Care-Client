...
export const useServerManagedDataTableFilters = ({
  ...
  const filterValues = useMemo<TanTableFilterValues>(() => {
    return definitions.reduce<TanTableFilterValues>((acc, definition) => {
      ...
      const rangeValue: TanTableRangeValue = {};
  ...
  const handleFilterChange = useCallback((filterId: string, value: TanTableFilterValue | undefined) => {
